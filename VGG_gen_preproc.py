...
class VGG(object):
    ...
    def fit(self, traingen, valgen, type):
        ...
        margin = 0.4 # default 0.4
        margin_upper = 0.7
        margin_lower = 0.1
        anchor_left , anchor_right= Contrastive(self.batch_size).pair_combos(embedding)
        labels= Contrastive(self.batch_size).get_binaray_labels(tfY)
        #cost  = Contrastive(self.batch_size).contrastive_loss_v1(labels , anchor_left , anchor_right, margin_lower , margin_upper)
        cost  = Contrastive(self.batch_size).contrastive_loss(labels , anchor_left , anchor_right, margin)
        #cost_tf = tf.contrib.losses.metric_learning.contrastive_loss(labels = labels, embeddings_anchor=anchor_left, embeddings_positive=anchor_right, margin = margin)
